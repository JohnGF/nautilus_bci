...
typedef struct _GDS_GHIAMP_CONFIGURATION
{
	...
	/*!
		...
		The counter runs over at 1,000,000 samples.
	 */
	BOOL CounterEnabled;
...
//! Measures impedance of all available synchronously sampled analog input channels for active or passive electrodes in ohm.
/*!
	<b>This method is deprecated. Please use \ref GDS_GHIAMP_GetImpedanceEx instead.</b>
	...
	The method will not fail if an invalid reference signal is measured on channel 1 (maybe due to a not connected electrode). Instead, appropriate error codes will be assigned to the other channels.
	...
	Values below 5,000 ohm or above 500,000 ohm represent different error codes and do not rate the severeness of error. Thus, they are not comparable to each other.
	The method returns quiet NaN values (\c std::numeric_limits<double>::quiet_NaN()) for channel 1, for unavailable channels and for channels that did not yield a finite impedance value.
	The availability of a channel can be determined using \ref GDS_GHIAMP_GetAvailableChannels.

	If a TMS box is connected, the caller must subtract 10 kilo-ohm from valid impedance values to get the correct value (do not do this for error codes).

	The following table lists the recommended classification of returned impedances for passive gel electrodes:

	\li \c < 5,000 for not measurable, estimated good impedance
	\li \c 5,000..500,000 measured impedance values
	\li \c > 500,000 for not measurable impedance due to an erroneous signal (maybe due to a not connected electrode, a too big offset, etc.)
	\li \c std::numeric_limits<double>::quiet_NaN() for channel 1 and for unavailable channels and for channels that yielded non-finite impedance values (such as NaN or infinity).
	...
	\li \c 3,000	The impedance could not be calculated but is estimated to be very low.
	\li \c 4,000	The amplitude of the measured signal almost equals the original signal. The impedance is estimated to be very low.
	\li \c 501,000	The amplitude of the measured signal is too small.
	\li \c 502,000	The offset of the measured signal is too large.
	\li \c 503,000	The RMS value of the measured signal is too large.
	\li \c 504,000	The low frequency power of the measured signal is too large.
	\li \c 505,000	The amplitude of the measured signal at channel 1 is too small.
	\li \c 506,000	The offset, RMS, or low frequency power at channel 1 is too large.
...
/*!
	Channel 1 and the ground electrode must always be connected for impedance measurement. A special z-electrode must be used on channel 1 if active electrodes are used.
	Active and passive electrodes cannot be mixed.

	Channel 1 plays a special role in impedance measurement and must always be connected. However, its impedance cannot be determined. Channel 1 will always receive NaN as impedance value.
	The method will not fail if an invalid reference signal is measured on channel 1 (maybe due to a not connected electrode). Instead, appropriate error codes will be assigned to the other channels.

	If a TMS electrode connector box is connected, set the \a tmsBoxCorrection parameter to \c true; 10 kilo-ohm will be subtracted of all measured impedance values in this case due to internal dropping resistors.
	Hence, no other type of electrode connector boxes can be connected to the same device if an TMS electrode connector box is connected for impedance measurement.
	...
	Negative values represent different error codes. The method returns quiet NaN values (\c std::numeric_limits<double>::quiet_NaN()) for channel 1, for unavailable channels and for channels that did not yield a finite impedance value.
	The availability of a channel can be determined using \ref GDS_GHIAMP_GetAvailableChannels.
	...
	\li \c -5	The RMS value of the measured signal is too big.
	\li \c -6	The drift of the measured signal is too big.
	\li \c -9	The amplitude of the measured signal at channel 1 is too small.
	\li \c -10	The offset, RMS, or drift at channel 1 is too big.
	\li \c std::numeric_limits<double>::quiet_NaN() for channel 1 and for unavailable channels and for channels that yielded non-finite impedance values (such as NaN or infinity).

	\param connectionHandle				The connection handle whose associated data acquisition session contains the device to measure impedance for.
	\param deviceName					A pointer to the name of the device to measure impedance for.
	\param electrodeType				The type of connected electrodes. All electrodes must be of the same selected type.
	\param tmsBoxCorrection				\c true if a TMS electrode connector box is connected and impedance values of all channels should be corrected by 10 kilo-ohm; otherwise, \c false.
	\param firstMeasurementOfSession	If this is the start of a new measurement session (i.e. the first in a series of consecutive calls to \ref getImpedanceEx), set this flag to \c true to clear internal interim values of previous measurements; for the following repeated calls, set this flag to \c false until the end of the measurement session.
	\param impedances					A pointer to an allocated array that receives the impedance values in ohm for each channel. An element at zero-based index <i>i</i> receives the impedance value in ohm of one-based channel number <i>i + 1</i>, or \c std::numeric_limits<double>::quiet_NaN(), or an error code (see remarks section of the method for details).

	\returns A \ref GDS_RESULT structure containing information about the operation's success.
*/
GDSCLIENTAPI_API GDS_RESULT GDS_GHIAMP_GetImpedanceEx(GDS_HANDLE connectionHandle, char(*deviceName)[DEVICE_NAME_LENGTH_MAX], GDS_ELECTRODE_TYPE electrodeType, BOOL tmsBoxCorrection, BOOL firstMeasurementOfSession, double(*impedances)[GDS_GHIAMP_CHANNELS_MAX]);
