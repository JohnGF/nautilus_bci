...
typedef struct _GDS_GUSBAMP_CONFIGURATION
{
	...
	/*!
		If short cut is enabled, a HIGH level on the SC input socket of the amplifier disconnects the electrodes from the amplifier input stage as long as the HIGH level is applied.
	...
	//! Indicates whether a sample counter should be applied on channel 16 instead of the measured signal if selected for acquisition (overrun at 1,000,000 samples).
	BOOL CounterEnabled;
	...
	/*!
		...
		In g.USBamp version 2.0 there is just one trigger line, so the values of the trigger channel can be 0 (LOW) or 250,000 (HIGH). 
...
//! Retrieves extended information about the specified g.USBamp device of the data acquisition session associated with the specified connection handle.
/*!
	...
	\param deviceInformation	A pointer to a fixed size array of a number of #GDS_GUSBAMP_DEVICE_INFORMATION_LENGTH_MAX characters that receives the device information string.

	\returns A \ref GDS_RESULT structure containing information about the operation's success.
 */
GDSCLIENTAPI_API GDS_RESULT  GDS_GUSBAMP_GetDeviceInformation(GDS_HANDLE connectionHandle, char (*deviceName)[DEVICE_NAME_LENGTH_MAX], char (*deviceInformation)[GDS_GUSBAMP_DEVICE_INFORMATION_LENGTH_MAX]);

//! Gets a list of supported sampling rates and their supported features.
/*!
	\a supportedSamplingRates is a pointer to the first element of an array that must be allocated by the caller before. To determine the number of elements of the array to allocate, call \ref GDS_GUSBAMP_GetSupportedSamplingRates with \a supportedSamplingRates set to #NULL.
	The function then returns the number of elements of the \a supportedSamplingRates array in \a supportedSamplingRatesCount. Allocate a \ref GDS_GUSBAMP_SAMPLING_RATE_FEATURES array of that size and pass it on to another call of \ref GDS_GUSBAMP_GetSupportedSamplingRates to retrieve the list of supported sampling rates.
	...
	\param supportedSamplingRates		A pointer to the first element of an allocated array of type \ref GDS_GUSBAMP_SAMPLING_RATE_FEATURES and size \a supportedSamplingRatesCount that receives the list of supported sampling rates, or #NULL to receive the number of supported sampling Rates in \a supportedSamplingRatesCount only.
	...
	\returns A \ref GDS_RESULT structure containing information about the operation's success.
 */
GDSCLIENTAPI_API GDS_RESULT  GDS_GUSBAMP_GetSupportedSamplingRates(GDS_HANDLE connectionHandle, char (*deviceName)[DEVICE_NAME_LENGTH_MAX], GDS_GUSBAMP_SAMPLING_RATE_FEATURES *supportedSamplingRates, size_t *supportedSamplingRatesCount);
...
/*!
	\a bandpassFilters is a pointer to an array that must be allocated by the caller before. To determine the number of elements of the array to allocate, call \ref GDS_GUSBAMP_GetBandpassFilters with \a bandpassFilters set to #NULL.
	...
	\returns A \ref GDS_RESULT structure containing information about the operation's success.
 */
GDSCLIENTAPI_API GDS_RESULT  GDS_GUSBAMP_GetBandpassFilters(GDS_HANDLE connectionHandle, char (*deviceName)[DEVICE_NAME_LENGTH_MAX], GDS_FILTER_INFO *bandpassFilters, size_t *bandpassFiltersCount);
...
/*!
	\a notchFilters is a pointer to an array that must be allocated by the caller before. To determine the number of elements of the array to allocate, call \ref GDS_GUSBAMP_GetNotchFilters with \a notchFilters set to #NULL.
	...
	\returns A \ref GDS_RESULT structure containing information about the operation's success.
 */
GDSCLIENTAPI_API GDS_RESULT  GDS_GUSBAMP_GetNotchFilters(GDS_HANDLE connectionHandle, char (*deviceName)[DEVICE_NAME_LENGTH_MAX], GDS_FILTER_INFO *notchFilters, size_t *notchFiltersCount);
...
/*!
	...
	\param deviceName			A pointer to the name of the device to get the asynchronous digital I/O channels's current values for.
	\param asyncDigitalIOs		A pointer to a fixed size array of a number of exactly \ref GDS_GUSBAMP_ASYNC_DIGITAL_IO_CHANNELS_MAX elements of type \ref GDS_GUSBAMP_ASYNC_DIGITAL_IO_CHANNEL that receives the current values of all provided asynchronous digital input and output channels.

	\returns A \ref GDS_RESULT structure containing information about the operation's success.
 */
GDSCLIENTAPI_API GDS_RESULT  GDS_GUSBAMP_GetAsyncDigitalIOs(GDS_HANDLE connectionHandle, char (*deviceName)[DEVICE_NAME_LENGTH_MAX], GDS_GUSBAMP_ASYNC_DIGITAL_IO_CHANNEL (*asyncDigitalIOs)[GDS_GUSBAMP_ASYNC_DIGITAL_IO_CHANNELS_MAX]);
...
/*!
	...
	\param deviceName				A pointer to the name of the device whose asynchronous digital output channel values should be set or changed.
	...
	\returns A \ref GDS_RESULT structure containing information about the operation's success.
 */
GDSCLIENTAPI_API GDS_RESULT  GDS_GUSBAMP_SetAsyncDigitalOutputs(GDS_HANDLE connectionHandle, char (*deviceName)[DEVICE_NAME_LENGTH_MAX], GDS_GUSBAMP_ASYNC_DIGITAL_IO_CHANNEL *asyncDigitalOutputs, size_t asyncDigitalOutputsCount);
...
/*!
	...
	\returns A \ref GDS_RESULT structure containing information about the operation's success.
 */
GDSCLIENTAPI_API GDS_RESULT  GDS_GUSBAMP_GetScaling(GDS_HANDLE connectionHandle, char (*deviceName)[DEVICE_NAME_LENGTH_MAX], GDS_GUSBAMP_SCALING *scaling);
...
/*!
	...
	\returns A \ref GDS_RESULT structure containing information about the operation's success.
 */
GDSCLIENTAPI_API GDS_RESULT  GDS_GUSBAMP_SetScaling(GDS_HANDLE connectionHandle, char (*deviceName)[DEVICE_NAME_LENGTH_MAX], GDS_GUSBAMP_SCALING *scaling);
...
/*!
	...
	\returns A \ref GDS_RESULT structure containing information about the operation's success.
 */
GDSCLIENTAPI_API GDS_RESULT  GDS_GUSBAMP_Calibrate(GDS_HANDLE connectionHandle, char (*deviceName)[DEVICE_NAME_LENGTH_MAX], GDS_GUSBAMP_SCALING *scaling);
...
/*!
	<b>This method is deprecated. Please use \ref GDS_GUSBAMP_GetImpedanceEx instead.</b>
	...
	\param deviceName		A pointer to the name of the device to use for impedance measurement.
	...
	\returns A \ref GDS_RESULT structure containing information about the operation's success.
 */
GDSCLIENTAPI_API GDS_RESULT  GDS_GUSBAMP_GetImpedance(GDS_HANDLE connectionHandle, char (*deviceName)[DEVICE_NAME_LENGTH_MAX], unsigned char channelNumber, double *impedance);
...
/*!
	...
	All returned impedance values are rounded to the nearest integral ohm value. Negative values represent different error codes.
	The following table lists the recommended classification of returned impedances and error codes for passive gel electrodes:

	\li \c < 7,500 for good impedance
	\li \c 7,500..20,000 for medium impedance
	\li \c > 20,000 for bad impedance

	Impedances for active electrodes connected via a g.GAMMAbox cannot be measured with g.USBamp devices.
	...
	\li \c -1	The impedance could not be calculated but is estimated to be very low.
	\li \c -2	The impedance could not be calculated but is estimated to be very low.
	\li \c -3	The impedance could not be calculated but is estimated to be very high.
	...
	\returns A \ref GDS_RESULT structure containing information about the operation's success.
*/
GDSCLIENTAPI_API GDS_RESULT  GDS_GUSBAMP_GetImpedanceEx(GDS_HANDLE connectionHandle, char(*deviceName)[DEVICE_NAME_LENGTH_MAX], unsigned char channelNumber, double *impedance);
