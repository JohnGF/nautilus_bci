...
typedef struct _GDS_GNAUTILUS_CONFIGURATION
{
	...
	/*!
		...
		Acceleration values range within +/- 6 g.
	 */
	...
	/*!
		...
		The counter value is the number of the current sample.
	 */
	BOOL Counter;
	...
	/*!
		...
		Link quality information values range within 0 and 100 percent.
	 */
	...
	/*!
		...
		Battery level values range within 0 and 100 percent.
	 */
	BOOL BatteryLevel;

	//! Indicates whether the synchronously sampled digital input channels should be included in data acquisition.
	/*!
		If enabled, the values of the digital trigger input channels come bit-encoded in a single additional channel appended to the battery level channel.
	 */
	BOOL DigitalIOs;
	...
	/*!
		...
		Valid validation indicator values are 0 and 1.
	 */
...
/*!
	...
	\param impedances		A pointer to an allocated array that receives the impedance values in ohm for each channel. An element at zero-based index <i>i</i> receives the impedance value in ohm of zero-based channel number <i>i</i>, or \c std::numeric_limits<double>::quiet_NaN() (see extended description of \ref GDS_GNAUTILUS_GetImpedanceEx/\ref GDS_GNAUTILUS_GetImpedance for details).
...
//! Sets the radio channel to use for wireless communication between headset and base station.
/*!
	...
	The network channel number specifies the channel that is used for communication between head set and base station.
	When switching on a g.Nautilus device, it pairs itself automatically to a base station using the first free channel selected by the base station.
	A free channel is a channel where no communication is running.
...
//! Retrieves the current radio channel used for communication between headset and base station.
/*!
...
//! Measures impedance of all available synchronously sampled analog input channels in ohm.
/*!
	<b>This method is deprecated. Please use \ref GDS_GNAUTILUS_GetImpedanceEx instead.</b>
	...
	All returned impedance values are rounded to the nearest integral kilo-ohm value. All measured values are clipped within a range of 5,000 ohm to 500,000 ohm.
	However, if the impedance value cannot be measured exactly, the method returns a value of 4,000 ohm for any estimated impedance value below 5,000 ohm, and 501,000 for any estimated impedance value above 500,000 ohm.
	The method returns quiet NaN values (\c std::numeric_limits<double>::quiet_NaN()) for unavailable channels and for channels that did not yield a finite impedance value. The availability of a channel can be determined using \ref GDS_GNAUTILUS_GetAvailableChannels.
...
/*!
	Electrode 'Cz' plays a special role in impedance measurement and must always be connected.
	The method will not fail if an invalid reference signal is measured on electrode 'Cz' (maybe due to a not connected electrode). Instead, appropriate error codes will be assigned to the other channels.

	All returned impedance values are rounded to the nearest integral ohm value in range. All measured values are clipped within a range of 600 ohm to 500,000 ohm.
	Negative values represent different error codes. The method returns quiet NaN values (\c std::numeric_limits<double>::quiet_NaN()) for unavailable channels and for channels that did not yield a finite impedance value.
	The availability of a channel can be determined using \ref GDS_GNAUTILUS_GetAvailableChannels.

	The following table lists the recommended classification of returned impedances for active gel electrodes:

	\li \c < 65,000 for good impedance
	\li \c 65,000..200,000 for medium impedance
	\li \c > 200,000 for bad impedance
	...
	\li \c -1	The amplitude of the measured signal almost equals the original signal. The impedance is estimated to be very low.
	\li \c -2	The impedance could not be calculated but is estimated to be very low.
	\li \c -3	The amplitude of the measured signal is too small. The impedance is estimated to be very high.
	\li \c -4	The offset of the measured signal is too big.
	\li \c -7	The shape of the measured signal does not match the expected one.
	\li \c -8	Too many invalid data frames were acquired during impedance measurement. This indicates bad wireless connection between the g.Nautilus headset and base station.
	\li \c -9	The amplitude of the measured signal at electrode 'Cz' is too small.
	\li \c -10	The offset of the measured signal at electrode 'Cz' is too big.
	\li \c -11	The shape of the measured signal at electrode 'Cz' does not match the expected one.
	\li \c -12	Too many invalid data frames were acquired for electrode 'Cz' during impedance measurement. This indicates bad wireless connection between the g.Nautilus headset and base station.
	\li \c std::numeric_limits<double>::quiet_NaN() for unavailable channels and for channels that yielded non-finite impedance values (such as NaN or infinity).
	...
	\returns A \ref GDS_RESULT structure containing information about the operation's success.
*/
GDSCLIENTAPI_API GDS_RESULT GDS_GNAUTILUS_GetImpedanceEx(GDS_HANDLE connectionHandle, char(*deviceName)[DEVICE_NAME_LENGTH_MAX], double(*impedances)[GDS_GNAUTILUS_CHANNELS_MAX]);
...
/*!
	<b>This method is deprecated. Please use \ref GDS_GNAUTILUS_StartImpedanceMeasurementEx instead.</b>
...
/*!
	This method returns immediately. The \a impedanceMeasurementCallback function will be called after each completed impedance measurement cycle until the \ref GDS_GNAUTILUS_StopImpedanceMeasurement method is called.
	If an error occurs during impedance measurement, it is forwarded to the callback as parameter. Errors occur in the same situations as for \ref GDS_GNAUTILUS_GetImpedanceEx.

	If the \c result parameter in the \ref GDS_GNAUTILUS_ImpedanceMeasurementCallback callback handler indicates an error and the \c aborted flag of the callback handler equals #TRUE as well, impedance measurement has been stopped automatically and can be started again with \ref GDS_GNAUTILUS_StartImpedanceMeasurementEx.
	If the \c result parameter in the \ref GDS_GNAUTILUS_ImpedanceMeasurementCallback callback handler indicates an error but the \c aborted flag of the callback handler equals #FALSE, only the current measurement cycle is affected but impedance measurement continues.
	...
	\returns A \ref GDS_RESULT structure containing information about the operation's success.
*/
GDSCLIENTAPI_API GDS_RESULT GDS_GNAUTILUS_StartImpedanceMeasurementEx(GDS_HANDLE connectionHandle, char(*deviceName)[DEVICE_NAME_LENGTH_MAX], GDS_GNAUTILUS_ImpedanceMeasurementCallback impedanceMeasurementCallback, void *userData);
