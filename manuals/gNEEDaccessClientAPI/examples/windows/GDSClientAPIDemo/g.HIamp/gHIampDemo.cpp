...
//-------------------------------------------------------------------------------------
void setup_config( GDS_GHIAMP_CONFIGURATION* cfg, unsigned int sample_rate );
void show_config( GDS_GHIAMP_CONFIGURATION* cfg );
...
{
	std::cout << "g.NEEDaccess: g.HIamp Demo" << std::endl << std::endl;
	...
	// Show all devices of interest (i.e. g.HIamp's).
	//-------------------------------------------------------------------------------------
	...
	for ( size_t i = 0; i < count_daq_units; i++ )
	{
		...
		{
			if ( connected_devices[i].ConnectedDevices[j].DeviceType == GDS_DEVICE_TYPE_GHIAMP )
			{
	...
	//-------------------------------------------------------------------------------------
	GDS_GHIAMP_CONFIGURATION* cfg_hiamp = new GDS_GHIAMP_CONFIGURATION;
	setup_config( cfg_hiamp, SAMPLE_RATE );
	GDS_CONFIGURATION_BASE* cfg = new GDS_CONFIGURATION_BASE[1];
	cfg[0].DeviceInfo.DeviceType = GDS_DEVICE_TYPE_GHIAMP;
	strcpy( cfg[0].DeviceInfo.Name, device_list[idx_selection].c_str() );
	cfg[0].Configuration = cfg_hiamp;
	...
	//-------------------------------------------------------------------------------------
	GDS_GHIAMP_CONFIGURATION* cfg_received_hiamp = (GDS_GHIAMP_CONFIGURATION*)(cfg_received[0].Configuration);
	std::cout << "Config of " << device_list[0] << ": " << std::endl;
	show_config( cfg_received_hiamp );

	// Set the data ready callback treshold.
	//-------------------------------------------------------------------------------------
	size_t data_ready_treshold_number_of_scans = std::max<size_t>( (size_t) std::floor((double)cfg_received_hiamp->SamplingRate * DATA_READY_TRESHOLD_MS/1000), cfg_received_hiamp->NumberOfScans );
	...
	try
	{
		...
		data_buffer = new float[buffer_size_in_samples];
		...
		while ( total_acquired_scans < total_scans_to_acquire )
		{
			...
			if ( ret.ErrorCode )
				throw std::logic_error("Error: GDS_GetData failed (" + std::string(ret.ErrorMessage) + std::string(")"));
...
void show_config( GDS_GHIAMP_CONFIGURATION* config )
{
	...
	std::cout << "Config.Channels[i].ReferenceChannel = ";
	for ( unsigned int i = 0; i < GDS_GHIAMP_CHANNELS_MAX; i++ )
	{
		if (config->Channels[i].Acquire)
			std::cout << config->Channels[i].ReferenceChannel << " "; 
