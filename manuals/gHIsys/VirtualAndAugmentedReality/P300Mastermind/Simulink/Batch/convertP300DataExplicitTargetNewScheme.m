function P_C = convertP300DataExplicitTargetNewScheme(P_C,NumRows,NumCols,mode,parafrequ)
...
y(end,-4 * npeaksamples + sort(breakends) + 1) = 1;
if mode > 0
    sections = find(diff(initline)>0);
    sections(end+1) = length(initline);
    lasttargetpattern = [];
    for sec = 1:length(sections)-1;
        targetstart = find(diff(y(end,sections(sec):sections(sec+1))) > 0)+sections(sec);
        targetend = find(diff(y(end,sections(sec):sections(sec+1))) < 0)+sections(sec);
        for ps = 1:length(targetstart)
            matchpattern = patterns(:,targetstart(ps):targetend(ps));
            if ~isempty(matchpattern)
                commonsym = intersect(lasttargetpattern,matchpattern(2:matchpattern(1,1)+1,1));
                if length(commonsym) ~= 1
                    lasttargetpattern = matchpattern(2:matchpattern(1,1)+1,1);
                    continue;
                end               
                y(end,sections(sec):sections(sec+1)) = y(end,sections(sec):sections(sec+1)) .* commonsym;
                lasttargetpattern = [];
                break;
            end
        end
