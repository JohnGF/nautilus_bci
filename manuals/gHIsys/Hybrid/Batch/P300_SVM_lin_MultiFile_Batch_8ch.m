...
%% Save data in correct format for SVM
...
if exist('AveragesForSVM_ClassifierData.mat') ~= 2
    features_train=train_SVM.X;
    classlabels_train=train_SVM.Y;

    save AveragesForSVM_ClassifierData features_train classlabels_train;
% If it already exist append data
else    
    load AveragesForSVM_ClassifierData.mat;
    AppendPos=size(features_train);
    AppendSize=size(train_SVM.X);
    features_train(:, AppendPos(2)+1:AppendPos(2)+AppendSize(2))=train_SVM.X;
    classlabels_train(:, AppendPos(2)+1:AppendPos(2)+AppendSize(2))=train_SVM.Y;
    save AveragesForSVM_ClassifierData features_train classlabels_train;
