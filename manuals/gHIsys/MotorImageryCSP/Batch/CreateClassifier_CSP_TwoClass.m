...
if NumTrials == 1                                           % data is not triggered yet        
    ...
    if (~isempty(MonFileName) && (~isempty(which(MonFileName))))
        ...
        P_C_triggered.MontageName = Mon.MontageName;
        
        P_C_triggered.XPosition = Mon.XPosition;
        P_C_triggered.YPosition = Mon.YPosition;
        P_C_triggered.ZPosition = Mon.ZPosition;        
    end    
    %% Filter    
...
FileName='classifier_CSP_TwoClasses.mat';
ProgressBarFlag=0;
C_O=gBSlinearclassifier(F_M,Method,P,TrainTestData,PlotFeatures,FileName,ProgressBarFlag);
...
%% plot r2d figures
if NumChannelsTriggered > 27
    plotOnly4CSPs(r2dCSP);
else
    gResult2d(r2dCSP);
end
gResult2d(r2dClassifier);
...
t=text(1,i,'Brain-Computer Interface Experiment: Demo','FontSize',18,'FontWeight','bold');
i=i-d-d;
...
t=text(1,i,'Realized with g.USBamp and g.BSanalyze.','FontSize',10,'FontWeight','bold');
